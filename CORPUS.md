...
Copyright © 2026 Genworks International; portions copyright © 2026 Gornskew Enterprises.  AGPL-3.0-or-later; see COPYING.txt.
...
every Readymacs image carries it as `lisply-index` on the path, so a
project's CI can also run it from that image:

```bash
docker run --rm --entrypoint lisply-index \
  -v "$PWD:/corpus:ro" -v "$PWD/out:/out" genworks/readymacs:devo-lite \
...
Other producers are welcome (the Readymacs console has an Emacs Lisp
...
(`POST /lisply/lisply-search`); the Readymacs console's
